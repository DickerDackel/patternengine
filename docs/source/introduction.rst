...
it's core, it's an infinite generator that creates booleans depending on time
and a provided pattern.

To instantiate a :class:`Heartbeat`, you provide 2 arguments:
...
.. important:: The coordinates returned by the ring are positioned around the
   ...
   responsibility to add the actual position of the object that comtains an
   emitter to the returned coordinates.
...
So a 3 segmentt 90 degree arc pointing straight down can be created by::
